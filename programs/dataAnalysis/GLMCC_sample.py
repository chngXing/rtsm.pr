...
class _Gk(_BaseGLM):
    ...
    def gk_second_derivative(self):
        ...
        if abs(self.theta[-1]) < 1.0e-3:
            vals = (self.tau / 2) * np.exp(self.theta[:self.m][self.areas[2]]) * self.func_f(
                -self.xk[self.areas[2]]) ** 2 * (1 - np.exp(-2 * self.delta / self.tau))
            vals = vals[:self.areas[0].sum()]
            d2gk_dj_ji2[self.areas[0]] = vals
        else:
...
class GLMCC(_Gk):
    ...
    def fit(self, t_sp, clm=0.01, eta=0.1, max_iter=1000, j_min=-3.0, j_max=5.0, verbose=True):
        ...
        self.theta[:self.m] = np.log((1.0 + np.sum(cc)) / (2 * self.w))
        self.theta[[-2, -1]] = 0.1
        iter_count = 0
    ...
    def _hessian(self):
        ...
        hessian[[-2, -1], :self.m] = hessian[:self.m, [-2, -1]].T
...
if __name__ == "__main__":
    ...
    tensor_data = df.to_numpy(dtype=np.float32)  # 或 torch.float64
    ...
    ax.set_title("GLMCC Fit Example")
    plt.tight_layout()
    plt.show()